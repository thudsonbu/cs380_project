...
function getTechIncident($techID) {
    ...
    if($con){
        ...
        try {
            ...
            $query = mysqli_prepare($con,
                "SELECT incidentID, customerID, productCode, dateOpened, title
                FROM incidents
                WHERE dateClosed IS NULL AND techID=?"
                );
            
            mysqli_stmt_bind_param($query, "s", $techID);
